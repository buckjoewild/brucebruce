...
- **SQLite Event Projection**: A derived read-model (`evidence/derived/events.db`) projected from append-only JSONL evidence logs by `tools/project_events_to_sqlite.py`. Runs at boot-time (before server.py) for incremental ingestion. Supports 5 predefined queries via `tools/query_sqlite.py` (recent, growth_story, security, hot_rooms, budget_compliance). The in-game `status` command includes an Event Pulse summary. No freeform SQL is exposed to clients. Documentation in `docs/QUERIES_SQLITE.md`.
