...
**Batch gotcha:** `for /f` may iterate multiple times if a command emits multiple lines. Counters must update once per check; prefer capturing output into a variable, then evaluating once.
